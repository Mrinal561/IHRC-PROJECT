...
interface LocationAutosuggestProps {
  value: string;
  locationId?: number; // optional, for initial selection
...
const LocationAutosuggest: React.FC<LocationAutosuggestProps> = ({ 
  value, 
  locationId, // not used in the component, but kept for consistency
  onChange, 
  districtId,
  label = "Location",
  ...
  const handleCreateLocation = async (locationName: string) => {
    ...
    try {
      ...
      onChange(locationName, newLocation.id); // return both value and locationId
      setIsOpen(false);
    } catch (error) {
  ...
  const handleSelect = (location: Location) => {
    onChange(location.name, location.id); // return both value and locationId
    setIsOpen(false);
  };
