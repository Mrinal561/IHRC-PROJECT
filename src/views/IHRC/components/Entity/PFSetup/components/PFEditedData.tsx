...
const PFEditedData: React.FC<PFEditedDataProps> = ({
    ...
    const fetchPFData = async () => {
        try {
            ...
            const transformedSignatoryData = response.signatory_data.map(
                (signatory: any) => ({
            ...
            const signatoryUsers = response.signatory_data.map(
                (signatory: any) => ({
    ...
    const handleSignatoryChange = (
        newSignatories: Array<{ value: string; label: string }>,
    ) => {
        ...
        const selectedUsers = newSignatories
            .map((sig) => users.find((user) => user.id === Number(sig.value)))
    ...
    const handleSubmit = async () => {
        try {
            ...
            const updateData = {
                ...
                register_certificate:
                    formData.register_certificate || undefined,
        ...
        <div className="border rounded-lg p-2 space-y-2">
          ...
          {selectedSignatories.map((signatory, index) => (
            ...
            <div className="grid grid-cols-1 gap-3">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                        PF Certificate(Accepted : Pdf/Zip/Image(Max Size: 20mb))
                    </label>
