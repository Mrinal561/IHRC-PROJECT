...
const AgreementTypeAutosuggest = ({ 
  ...
  // New function to fetch all agreement types
  const fetchAllAgreementTypes = async () => {
    setIsLoading(true);
    try {
      const { data } = await httpClient.get(endpoints.common.agreementTypeSuggestions());
      const agreementTypesData = Array.isArray(data) ? data : [data];
  ...
  // Initial fetch of all agreement types
  useEffect(() => {
    fetchAllAgreementTypes();
  }, []);

  // Modified search effect to filter existing agreement types or fetch new ones
  useEffect(() => {
    const searchAgreementTypes = async () => {
      if (!value) {
        setFilteredAgreementTypes(agreementTypes);
        return;
      }
