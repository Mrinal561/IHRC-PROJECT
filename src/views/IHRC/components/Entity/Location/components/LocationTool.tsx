...
const LocationTool: React.FC = () => {
    ...
    const companyGroupOptions = useMemo(() => {
        const groups = [
            ...new Set(entityDataSet.map((item) => item.Company_Group_Name)),
        ]
    ...
    const filteredDistrictOptions = useMemo(() => {
        if (!selectedCompanyGroup || !selectedCompanyName || !selectedState)
            return []
        ...
        return [...new Set(districts)].map((district) => ({
            value: district!,
            label: district!,
        }))
    ...
    const onDialogOk = () => {
        if (
            ...
            showSuccessToast(
                `Location "${locationName.trim()}" has been successfully added.`,
            )
            onDialogClose()
        } else {
            showFailToast(
                'Please enter a valid Location Name, select a Company Group, Company Name, State, and District.',
            )
        }
    }
