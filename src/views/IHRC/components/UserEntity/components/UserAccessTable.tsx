...
const MenuItemBox = ({ menu, parentColors }) => {
    if (!menu.access.can_list) return null;
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            {menu.children && menu.children.length > 0 && (
                <div className="flex items-center gap-2">
                    {menu.children.some(child => child.access.can_list) && <div className="text-gray-400">→</div>}
                    {menu.children.map((childMenu, idx) => (
                        childMenu.access.can_list && (
                            <div key={idx} className="flex items-center gap-2">
                                ...
                                {childMenu.children && childMenu.children.length > 0 && (
                                    <>
                                        {childMenu.children.some(gc => gc.access.can_list) && <div className="text-gray-400">→</div>}
                                        {childMenu.children.map((grandChild, index) => (
                                            grandChild.access.can_list && (
                                                <MenuItemBox 
                                                    key={index} 
                                                    menu={grandChild} 
...
const ModuleMenuGroup = ({ module, colors }) => {
    const accessibleMenus = module.menus.filter(menu => {
        const hasAccessibleChildren = menu.children?.some(child => 
            child.access.can_list || 
            child.children?.some(grandChild => grandChild.access.can_list)
        );
        return menu.access.can_list || hasAccessibleChildren;
    });
                        ...
                        <div className="flex flex-col">
                            {modules.map((module, idx) => {
                                ...
                                const hasAccessibleMenus = module.menus.some(menu => 
                                    ...
                                    menu.children?.some(child => child.access.can_list)
                                );
