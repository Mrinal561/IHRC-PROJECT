...
const ChallanVisualization: React.FC<ChallanVisualizationProps> = ({ companyId }) => {
  ...
  const transformChartData = (data:any['data']) => {
    ...
    const transformedSeries = categories.map(category => {
      ...
      return {
        name: category,
        data: data.labels.map((_, monthIndex) => 
  ...
  const fetchGraphData = async () => {
    ...
    try {
      const { data } = await httpClient.get<any>(
        endpoints.graph.analytics(selectedType.value, selectedCode.label, financialYear)
      );
      
      if (data.success && data.data) {
        const transformedSeries = transformChartData(data.data);
        setChartData({
  ...
  const getEndpointForType = (type: CodeType) => {
    switch (type) {
      case 'PF': return `${endpoints.pfSetup.getAllCodes()}`;
      case 'ESI': return `${endpoints.esiSetup.getAllCodes()}`;
      case 'LWF': return `${endpoints.lwfSetup.getAllCodes()}`;
      case 'PTEC': return `${endpoints.ptSetup.getAllCodes()}`;
      case 'PTRC': return `${endpoints.ptSetup.rcCodes()}`;
      default: return `${endpoints.pfSetup.getAllCodes()}`;
    }
  };
