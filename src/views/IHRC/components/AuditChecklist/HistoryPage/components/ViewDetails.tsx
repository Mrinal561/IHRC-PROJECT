...
interface ComplianceDetailData {
    id: number;
    uuid: string;
    record_id: string;
    status: string;
...
const ComplianceDetails = () => {
    ...
    const complianceData = location.state as ComplianceDetailData | undefined
    const compliance = complianceData?.compliance_detail
    ...
    useEffect(() => {
        ...
        fetchStateName()
    }, [compliance?.state_id, compliance?.scope])
    ...
    return (
        <AdaptableCard className="p-4">
                ...
                <div className="flex gap-6">
                    ...
                    <div className="w-1/2">
                        ...
                        <p className="text-sm mb-2">
                            <strong>Default Due Date:</strong>{' '}
                            {formatDate(compliance.default_due_date?.first_date)} - {formatDate(compliance.default_due_date?.last_date)}
                        </p>
                        {/* <p className="text-sm mb-2"><strong>Proof Mandatory:</strong> {compliance.proof_mandatory ? 'Yes' : 'No'}</p> */}
                        <p className="text-sm mb-2"><strong>Approval Required:</strong> {compliance.approval_required ? 'Yes' : 'No'}</p>
                        <p className="text-sm mb-2"><strong>Created Type:</strong> {compliance.created_type}</p>
            ...
            {/* Remarks Section */}
            {complianceData?.AssignedComplianceRemark?.length > 0 && (
                <div className="border p-4 rounded-md mt-6">
                    <h3 className="text-xl font-semibold mb-4">Remarks</h3>
                    {complianceData.AssignedComplianceRemark.map((remark) => (
                        <div key={remark.id} className="mb-2">
                            <p className="text-sm"><strong>Remark:</strong> {remark.remark}</p>
                            <p className="text-sm text-gray-500">Created at: {formatDate(remark.created_at)}</p>
                        </div>
                    ))}
                </div>
            )}
